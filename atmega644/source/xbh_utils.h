...
#define XBH_DEBUG xbh_debug_write
#define XBH_WARN  xbh_warn_write
#define XBH_ERROR xbh_error_write

#define xbh_debug_write(format, args...) xbh_debug_writeP(PSTR(format), ## args )
#define xbh_warn_write(format, args...)  { xbh_debug_time(); xbh_debug_writeP(PSTR("warning: "format), ## args); }
#define xbh_error_write(format, args...) { xbh_debug_time(); xbh_debug_writeP(PSTR("ERROR: "format), ## args); }
...
inline char *constStringToBuffer(u08 *dest, const prog_char * src)
{
  return strcpy_P((char *) dest, src);
}

inline char *constStringToBufferN(u08 *dest, const prog_char * src, size_t n)
{
  return strncpy_P((char *) dest, src, n);
}
