...
Addons :  USE_LCD_4Bit wird zur Ansteuerung f�r Paraelle Ansteuerung benutzt (Ralf Figge)
------------------------------------------------------------------------------*/
...
#endif //USE_LCD_4Bit
//------------------------------------------------------------------------------
void lcd_vprint_P (unsigned char zeile,unsigned char spalte,const char *Buffer, va_list *ap)
{
	int format_flag;
	char str_buffer[10];
	...
	//Berechnet Adresse f�r die Zeile und schreibt sie ins DD-Ram
...
void lcd_print_P (unsigned char zeile,unsigned char spalte,const char *Buffer,...)
{
	va_list ap;
	va_start (ap, Buffer);	
	lcd_vprint_P(zeile,spalte,Buffer, &ap);
	va_end(ap);
}
