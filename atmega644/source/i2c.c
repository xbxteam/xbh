...
volatile unsigned long i2cEntryTime;
volatile unsigned long i2cTimeOut=1000000;
...
inline void i2cWaitForComplete(void)
{
	...
	{
		if(time > (i2cEntryTime + i2cTimeOut)) return;
	}
}
...
u08 i2cMasterSendNI(u08 deviceAddr, u08 length, u08* data)
{
	...
	if( inb(TWSR) == TW_MT_SLA_ACK)
	{
		...
		while(length)
		{
			i2cSendByte( *data++ );
			i2cWaitForComplete();
			if(time > (i2cEntryTime + i2cTimeOut)) break;
			length--;
		}
	}
	else
