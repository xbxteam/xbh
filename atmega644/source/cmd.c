...
extern unsigned char * xbd_ip_bytes;
...
COMMAND_STRUCTUR COMMAND_TABELLE[] = // Befehls-Tabelle
{
	...
	PROGMEM char helptext[] = {
		...
		"NTP     - list/change NTP\r\n"
		"NTPR    - NTP Refresh\r\n"
		"MAC     - list/change MAC-address\r\n"
		"XBDIP   - list/change XBD IP for IPCOMM\r\n"
		"VER     - list enc version number\r\n"
		"SV      - set variable\r\n"
                "PING    - send Ping\r\n"
                "XBDPING - send Ping to xbd\r\n"
                #if USE_MAIL
                "MAIL   - send E_MAIL\r\n"
                #endif //USE_MAIL
		#if USE_WOL
...
unsigned char extract_cmd (char *string_pointer)
{
	...
	if(!strcasecmp("MAC",string_pointer_tmp))
	{
		for (unsigned char a = 0; a<MAX_VAR;a++)
		{ 
	        string_pointer_tmp = strsep(&string_pointer,"-: ");  
			variable[a] = strtol(string_pointer_tmp,NULL,16);
		}	
	}
    else
	{
...
void command_xbdip (void)
{
	write_eeprom_ip(XBDIP_EEPROM_STORE);
	(*((unsigned long*)&xbd_ip_bytes[0])) = get_eeprom_value(XBDIP_EEPROM_STORE,DEFAULT_XBD_IP);
	usart_write("XBD IP: %1i.%1i.%1i.%1i\r\n",xbd_ip_bytes[0],xbd_ip_bytes[1],xbd_ip_bytes[2],xbd_ip_bytes[3]);
}
...
void command_mac (void)
{
	...
	{	
		...
		{
			eeprom_busy_wait ();
			eeprom_write_byte((unsigned char *)(XBH_MAC_EEPROM_STORE + count),variable[count]);
			eeprom_mac[count]=(unsigned char)variable[count];
		}
	}
	
	usart_write("XBH MAC: %2x:%2x:%2x:%2x:%2x:%2x\r\n",eeprom_mac[0],eeprom_mac[1],eeprom_mac[2],eeprom_mac[3],eeprom_mac[4],eeprom_mac[5]);
}
...
void command_xbdping(void)
{
  u32 ip=get_eeprom_value(XBDIP_EEPROM_STORE,DEFAULT_XBD_IP);
  (*((unsigned long*)&ping.ip1[0]))=ip;
  usart_write("Sending ping to XBD\n");
...
void read_mac_address()
{
	...
	{
		eeprom_busy_wait ();
		eeprom_mac[count]=eeprom_read_byte((unsigned char *)(XBH_MAC_EEPROM_STORE + count));
	}
}
